...
export default function AdminPreviewAuditLogsPage() {
  return (
    <section className="space-y-4">
      <header className="rounded-2xl border border-[#dbe4f4] bg-white px-5 py-4 shadow-sm">
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-[#1f2d45]">Activity Audit Trail (Preview)</h1>
      </header>
          ...
          <table className="min-w-full text-left text-sm text-[#2b3e60]">
            <thead className="text-xs font-semibold uppercase tracking-[0.08em] text-[#7a8ca8]">
              <tr className="border-b border-[#edf2f9]">
                <th className="py-2 pr-4">Actor</th><th className="py-2 pr-4">Action</th><th className="py-2 pr-4">Target</th><th className="py-2 pr-4">Timestamp</th><th className="py-2">Severity</th>
              </tr>
            </thead>
            <tbody>
              {auditRows.map((row) => (
                <tr key={`${row.action}-${row.timestamp}`} className="border-b border-[#edf2f9] last:border-0">
                  ...
                  <td className="py-3"><span className={`rounded-md px-2 py-1 text-xs font-semibold ${severityClass[row.severity]}`}>{row.severity}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
